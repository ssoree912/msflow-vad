...
def single_parallel_flows(c_feat, c_cond, n_block, clamp_alpha, spatial_hw, subnet=subnet_conv_ln):
    ...
    print('Build parallel flows: channels:{}, block:{}, cond:{}, hw:{}'.format(c_feat, n_block, c_cond, spatial_hw))
...
def build_msflow_model(c, c_feats, spatial_hw_list):
    c_conds = c.c_conds
    n_blocks = c.parallel_blocks
    clamp_alpha = c.clamp_alpha
    parallel_flows = []
    for c_feat, c_cond, n_block, hw in zip(c_feats, c_conds, n_blocks, spatial_hw_list):
    ...
    for idx, (c_feat, hw) in enumerate(zip(c_feats, spatial_hw_list)):
        h, w = hw
